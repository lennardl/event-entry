...
export function EventOperationsApp({ initialState = null, initialError = null }: { initialState?: AppState | null; initialError?: string | null }) {
  ...
  const role: Role = state?.role ?? "Command Centre Viewer";
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="role-switcher"><span>Signed in as</span><strong>{role}</strong></div>
    ...
    <section>
      ...
      <div className="dashboard-grid">
        ...
        <article className="panel">
          <div className="panel-title"><div><span className="eyebrow">Gate activity</span><h2>Recent entry totals</h2></div><span className="subtle">From recorded scans</span></div>
          <div className="gate-cards">
            {state.gates.map((gate) => {
              ...
              return <div className="gate-card" key={gate.id}><div><span className="status-dot" /><strong>{gate.name}</strong></div><span>{gateEntries} entered</span><small>{state.scans.some((scan) => scan.gateId === gate.id) ? "Activity recorded" : "No recent activity"}</small></div>;
            })}
          </div>
          <div className="queue-note"><strong>Device connectivity</strong><span>Use scanner access history to verify individual devices</span></div>
        </article>
      </div>

      <article className="panel activity-panel">
        <div className="panel-title"><div><span className="eyebrow">Latest activity</span><h2>Entry feed</h2></div><span className="subtle">Updated on refresh</span></div>
...
function EventSetup({ state, onEventCreated }: { state: AppState; onEventCreated: (eventId: string) => Promise<void> }) {
  ...
  const [eventQuery, setEventQuery] = useState(""); const [statusFilter, setStatusFilter] = useState("active"); const [eventPage, setEventPage] = useState(1);
  const router = useRouter();
  const refresh = () => onEventCreated(state.event.id);
  const matchingEvents = useMemo(() => state.events.filter((event) => (statusFilter === "all" || statusFilter === "deleted" ? statusFilter !== "deleted" || Boolean(event.deletedAt) : !event.deletedAt && (statusFilter === "active" || event.status === statusFilter)) && `${event.name} ${event.venue}`.toLowerCase().includes(eventQuery.toLowerCase())), [eventQuery, state.events, statusFilter]);
  const visibleEvents = matchingEvents.slice((eventPage - 1) * 5, eventPage * 5); const eventPages = Math.max(1, Math.ceil(matchingEvents.length / 5));
  return <section>
    ...
    <article className="panel event-list"><div className="panel-title"><div><span className="eyebrow">All events</span><h2>{matchingEvents.length} found</h2></div></div><div className="toolbar"><input aria-label="Search events" placeholder="Search name or venue" value={eventQuery} onChange={(event) => { setEventQuery(event.target.value); setEventPage(1); }} /><select aria-label="Filter event status" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setEventPage(1); }}><option value="active">All active</option><option value="draft">Draft</option><option value="live">Live</option><option value="closed">Closed</option><option value="archived">Archived</option><option value="deleted">Recently deleted</option><option value="all">Everything</option></select></div>{visibleEvents.map((event) => <div key={event.id} className={event.id === state.event.id ? "event-list-row active" : "event-list-row"}><div><strong>{event.name}</strong><span>{event.venue} · {event.startDate}{event.endDate !== event.startDate ? `–${event.endDate}` : ""}</span></div><span className={`status-label ${event.status === "live" ? "allowed" : ""}`}>{event.deletedAt ? "deleted" : event.status}</span><span>{compactNumber(event.ticketCount)} tickets · {compactNumber(event.admitted)} admitted</span>{event.deletedAt ? state.role === "Super Admin" ? <button onClick={async () => { await requestAction({ action: "restoreEvent", eventId: event.id }); await onEventCreated(event.id); }}>Restore</button> : null : event.id === state.event.id ? <strong>Active</strong> : <button onClick={() => void onEventCreated(event.id)}>Open</button>}</div>)}<div className="modal-actions"><button disabled={eventPage === 1} onClick={() => setEventPage((page) => page - 1)}>Previous</button><span>Page {eventPage} of {eventPages}</span><button disabled={eventPage >= eventPages} onClick={() => setEventPage((page) => page + 1)}>Next</button></div></article>
    <div className="setup-grid">
      ...
      {state.role === "Super Admin" ? <AccessManagement /> : null}
    ...
    {selection === "lifecycle" ? <LifecycleDialog event={state.event} readiness={state.readiness} canDelete={state.role === "Super Admin"} onClose={() => setSelection(null)} onSaved={refresh} /> : null}
...
function EventDetailsDialog({ event, onClose, onSaved }: { event: AppState["event"]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(formEvent: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await requestAction({ action: "updateEvent", eventId: event.id, expectedVersion: event.version, name: form.get("name"), venue: form.get("venue"), capacity: Number(form.get("capacity")), startDate: form.get("startDate"), endDate: form.get("endDate"), timeZone: form.get("timeZone"), doorsOpen: form.get("doorsOpen"), entryWindowStart: form.get("entryWindowStart"), entryWindowEnd: form.get("entryWindowEnd"), eventEnd: form.get("eventEnd") });
      if (!result.updated) throw new Error("This event changed elsewhere, or its capacity conflicts with issued allocations. Refresh and try again.");
      await onSaved();
...
function LifecycleDialog({ event, readiness, canDelete, onClose, onSaved }: { event: AppState["event"]; readiness: AppState["readiness"]; canDelete: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const [confirmation, setConfirmation] = useState(canDelete ? "" : "__not_authorized__");
  ...
  async function remove() { setBusy(true); setError(null); try { const result = await requestAction({ action: "deleteEvent", eventId: event.id, confirmation }); if (!result.deleted) throw new Error("Deletion requires an archived event and its exact name."); window.history.replaceState(null, "", "/?view=events"); window.location.reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Event could not be deleted"); setBusy(false); } }
  return <div className="modal-backdrop"><div className="modal" role="dialog" aria-modal="true" aria-labelledby="lifecycle-title"><button className="drawer-close" onClick={onClose} aria-label="Close">×</button><span className="eyebrow">Lifecycle</span><h2 id="lifecycle-title">Event status: {event.status}</h2><p>Live events admit tickets. Closed and archived events retain their complete ledger.</p>{!readiness.ready ? <div className="inline-message">{readiness.checks.filter((check) => !check.ok).map((check) => check.label).join(" · ")}</div> : null}{error ? <div className="inline-message error">{error}</div> : null}<div className="lifecycle-actions"><button disabled={busy || event.status === "draft"} onClick={() => void change("draft")}>Move to draft</button><button disabled={busy || event.status === "live" || !readiness.checks.filter((check) => check.id !== "status").every((check) => check.ok)} onClick={() => void change("live")}>Publish live</button><button disabled={busy || event.status === "closed"} onClick={() => void change("closed")}>Close event</button><button className="danger-button" disabled={busy || event.status === "archived"} onClick={() => void change("archived")}>Archive</button></div>{event.status === "archived" ? <div className="destructive-zone"><p>This moves the event to Recently deleted without erasing its {event.capacity.toLocaleString()} capacity configuration or operational history. It can be restored.</p><label><span>Type <strong>{event.name}</strong> to confirm</span><input value={confirmation} onChange={(changeEvent) => setConfirmation(changeEvent.target.value)} /></label><button className="danger-button" disabled={busy || confirmation !== event.name} onClick={() => void remove()}>Move to Recently deleted</button></div> : null}</div></div>;
}
...
function AccessManagement() {
  type User = { email: string; role: Role; enabled: boolean; sessionVersion: number; lastLoginAt: string | null };
  const [open, setOpen] = useState(false); const [users, setUsers] = useState<User[]>([]); const [historyCount, setHistoryCount] = useState(0); const [error, setError] = useState<string | null>(null);
  async function load() { setError(null); const response = await fetch("/api/admin/users", { cache: "no-store" }); const data = await response.json(); if (!response.ok) return setError(data.error ?? "Access data unavailable"); setUsers(data.users); setHistoryCount(data.loginHistory.length); }
  async function update(email: string, change: { role?: Role; enabled?: boolean; revokeSessions?: boolean }) { const response = await fetch("/api/admin/users", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, ...change }) }); if (!response.ok) { const data = await response.json(); return setError(data.error ?? "User update failed"); } await load(); }
  return <article className="panel setup-card"><span className="eyebrow">Access control</span><h2>Operators and sessions</h2><p>Manage roles, disable access, revoke sessions and inspect login activity.</p><strong>{users.length ? `${users.length} operators · ${historyCount} recent logins` : "Database-backed access"}</strong><button className="secondary-button" onClick={() => { setOpen(true); void load(); }}>Manage access</button>{open ? <div className="modal-backdrop"><div className="modal manager-modal"><button className="drawer-close" onClick={() => setOpen(false)} aria-label="Close">×</button><span className="eyebrow">Security administration</span><h2>Operators and active sessions</h2>{error ? <div className="inline-message error">{error}</div> : null}<div className="access-user-list">{users.map((user) => <div key={user.email}><div><strong>{user.email}</strong><small>{user.lastLoginAt ? `Last login ${new Date(user.lastLoginAt).toLocaleString("en-SG")}` : "Never signed in"}</small></div><select value={user.role} onChange={(event) => void update(user.email, { role: event.target.value as Role, revokeSessions: true })}>{roles.map((role) => <option key={role}>{role}</option>)}</select><button className="secondary-button" onClick={() => void update(user.email, { revokeSessions: true })}>Revoke sessions</button><button className={user.enabled ? "danger-button" : "secondary-button"} onClick={() => void update(user.email, { enabled: !user.enabled, revokeSessions: true })}>{user.enabled ? "Disable" : "Enable"}</button></div>)}</div></div></div> : null}</article>;
}
