...
function EventSetup({ state, onEventCreated }: { state: AppState; onEventCreated: (eventId: string) => Promise<void> }) {
  ...
  return <section>
    ...
    <div className="setup-grid">
      ...
      <article className="panel setup-card readiness-card"><span className="eyebrow">Readiness</span><h2>{state.readiness.ready ? "Ready for gates" : "Needs attention"}</h2><div className="readiness-list">{state.readiness.checks.map((check) => <div key={check.id}><span>{check.ok ? "✓" : "!"}</span><div><strong>{check.label}</strong><small>{check.detail}</small></div></div>)}</div><button className="secondary-button" onClick={() => router.push(`/?view=overview&event=${encodeURIComponent(state.event.id)}`)}>Open command overview</button></article>
    </div>
...
function EventDetailsDialog({ event, onClose, onSaved }: { event: AppState["event"]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(formEvent: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await requestAction({ action: "updateEvent", eventId: event.id, name: form.get("name"), venue: form.get("venue"), capacity: Number(form.get("capacity")), entryWindowStart: form.get("entryWindowStart"), entryWindowEnd: form.get("entryWindowEnd") });
  ...
  return <div className="modal-backdrop"><form className="modal event-form" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="event-details-title"><button type="button" className="drawer-close" onClick={onClose} aria-label="Close">×</button><span className="eyebrow">Event details</span><h2 id="event-details-title">Edit {event.name}</h2><div className="form-grid"><label><span>Name</span><input name="name" required defaultValue={event.name} /></label><label><span>Venue</span><input name="venue" required defaultValue={event.venue} /></label><label><span>Capacity</span><input name="capacity" type="number" min={1} max={250000} required defaultValue={event.capacity} /></label><label><span>Entry starts</span><input name="entryWindowStart" type="time" required defaultValue={event.entryWindowStart} /></label><label><span>Entry ends</span><input name="entryWindowEnd" type="time" required defaultValue={event.entryWindowEnd} /></label></div>{error ? <div className="inline-message error">{error}</div> : null}<div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy}>{busy ? "Saving…" : "Save event"}</button></div></form></div>;
}
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await requestAction({
        ...
        capacity: Number(form.get("capacity")), entryWindowStart: form.get("entryWindowStart"), entryWindowEnd: form.get("entryWindowEnd"), zoneCount: Number(form.get("zoneCount")), gateCount: Number(form.get("gateCount")),
      });
